...
public class FraudDetectionJob {
	...
	public static void main(String[] args) throws Exception {
		...
		// Use the default sliding window assigner
		SlidingEventTimeWindows defaultSlidingWindow = SlidingEventTimeWindows.of(Time.seconds(5), Time.seconds(1));
		TumblingEventTimeWindows defaultTumblingWindow = TumblingEventTimeWindows.of(Time.minutes(5));

		CustomSlidingWindow customSlidingWindow1 = new CustomSlidingWindow(5000,2000);
		CustomSlidingWindow customSlidingWindow2 = new CustomSlidingWindow(5000,1000);

		CustomTumblingWindow customTumblingWindow1 = new CustomTumblingWindow(3000,2000);
		CustomTumblingWindow customTumblingWindow2= new CustomTumblingWindow(5000,1000);

		// Use a custom sliding window assigner
//		CustomSlidingWindow customSlidingWindow = new CustomSlidingWindow(3000, 100);
...
//		DataStream<Tuple2<Long, Double>> keyedTransactions2 = transactions1
//				.keyBy(Transaction::getAccountId)
//				.process(TimeStampAssigner1)
//				.name("keyed-transactions-2")
//				.setParallelism(1);

		long startTime= TimeStampAssigner1.getStartTime();
//		CustomTumblingWindow customTumblingWindow = new CustomTumblingWindow(5000,false, 100);
		keyedTransactions1.print("keyed-transaction");

		// Use a custom window assigner to aggregate transactions per account
		DataStream<Tuple2<Long, Double>> customSlidingWindowAggregatedTransactions = keyedTransactions1
				.keyBy(0)
//				.window(new CustomSlidingWindow(3000,1000))
//				.window(new CustomTumblingWindow(5000,1000))
				.window(customSlidingWindow1)
				.aggregate(new IncrementalMeanAggregate())
				.name("aggregated-transactions-custom")
				.setParallelism(1)
				;
//		aggregatedTransactions1.print("aggregated-transaction");


//		DataStream<Tuple2<Long, Double>> defaultSlidingWindowAggregatedTransactions = keyedTransactions1
//				.keyBy(0)
//				.window(defaultSlidingWindow)
//				.aggregate(new IncrementalMeanAggregate())
//				.name("aggregated-transactions-default")
//				.setParallelism(1);

		// Use a custom window assigner to aggregate transactions per account
		DataStream<Tuple2<Long, Double>> customSlidingWindowAggregatedTransactions1 = keyedTransactions1
				.keyBy(0)
//				.window(new CustomSlidingWindow(3000,1000))
//				.window(new CustomTumblingWindow(5000,1000))
		...
		FraudDetector2 customSlidingfraudDetector = new FraudDetector2(startTime);
		FraudDetector1 customSlidingfraudDetector1 = new FraudDetector1(startTime);

		DataStream<Alert> alerts2 = customSlidingWindowAggregatedTransactions
				.keyBy(tuple -> tuple.f0)
				.process(customSlidingfraudDetector)
				.name("fraud-detector-2")
				.setParallelism(1);
		alerts2.print("alerts2");

		DataStream<Alert> alerts1 = customSlidingWindowAggregatedTransactions1
				.keyBy(tuple -> tuple.f0)
				.process(customSlidingfraudDetector1)
				.name("fraud-detector-1")
				.setParallelism(1);
		alerts1.print("alerts1");



		alerts1.addSink(new AlertSink()).name("send-alerts-1");
		alerts1.addSink(new AlertSink()).name("send-alerts-2");
//
