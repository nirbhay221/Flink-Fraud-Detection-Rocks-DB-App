...
public class FraudDetectionJob {
	...
	public static void main(String[] args) throws Exception {
		...
		SingleOutputStreamOperator<Tuple2<Long, Double>> aggregatedTransactions1 = keyedTransactions1
				.keyBy(0)
				.window(customSlidingWindow)
				.process(new CustomMeanProcessWindowFunction(customSlidingWindow))
	...
	public static class CustomMeanProcessWindowFunction extends ProcessWindowFunction<Tuple3<Long, Double, Long>, Tuple2<Long, Double>, Tuple, TimeWindow> {

		private final CustomSlidingWindow customSlidingWindow;

		public CustomMeanProcessWindowFunction(CustomSlidingWindow customSlidingWindow) {
			this.customSlidingWindow = customSlidingWindow;
		}
		...
		public void process(Tuple key, Context context, Iterable<Tuple3<Long, Double, Long>> elements, Collector<Tuple2<Long, Double>> out) throws Exception {
			...
			Tuple2<Long, Double> combinedPanes = customSlidingWindow.combinePanes(window);
			if (combinedPanes != null) {
				out.collect(combinedPanes);
			}
		}
	}
	...
	public static class IncrementalMeanAggregate implements AggregateFunction<Tuple3<Long, Double, Long>, Tuple3<Long, Double, Long>, Tuple2<Long, Double>> {

		private final Map<Long, List<Double>> paneElementsMap;
		private final CustomSlidingWindow customSlidingWindow;

		public IncrementalMeanAggregate(Map<Long, List<Double>> paneElementsMap, CustomSlidingWindow customSlidingWindow) {
			this.paneElementsMap = paneElementsMap;
			this.customSlidingWindow = customSlidingWindow;
		}
		...
		@Override
		public Tuple3<Long, Double, Long> add(Tuple3<Long, Double, Long> value, Tuple3<Long, Double, Long> accumulator) {
			return new Tuple3<>(accumulator.f0 + 1, accumulator.f1 + value.f1, value.f2);
		}
		...
		public Tuple2<Long, Double> getResult(Tuple3<Long, Double, Long> accumulator) {
			...
			TimeWindow window = new TimeWindow(accumulator.f2, accumulator.f2 + customSlidingWindow.size);
			Tuple2<Long, Double> combined = customSlidingWindow.combinePanes(window);
			if (combined != null) {
				return combined;
			} else {
		...
		public Tuple3<Long, Double, Long> merge(Tuple3<Long, Double, Long> a, Tuple3<Long, Double, Long> b) {
			return new Tuple3<>(a.f0 + b.f0, a.f1 + b.f1, Math.min(a.f2, b.f2));
		}
	}
